...
class VectorQuantizer2(nn.Module):
    ...
    def __init__(
        self, vocab_size, Cvae, beta: float = 0.25,
        ...
        self.register_buffer('ema_cluster_size', torch.zeros(self.vocab_size)) # activation counter
        self.ema_w = nn.Parameter(torch.Tensor(self.vocab_size, self.Cvae))    # EMA weight of embedding
    ...
    def extra_repr(self) -> str:
        return f'{self.v_patch_nums}, beta={self.beta}  |  S={len(self.v_patch_nums)}, quant_resi={self.quant_resi_ratio}'
    ...
    def forward(self, f_BChw: torch.Tensor, ret_usages = False) -> Tuple[torch.Tensor, List[float], torch.Tensor]:
        ...
        with torch.amp.autocast( 'cuda', enabled=False):
            ...
            for index, pn in enumerate(self.v_patch_nums): # from small to large
                ...
                encoding_indices = idx_N.unsqueeze(1)  # [B*pn²] -> [B*pn², 1]
                encodings = torch.zeros(encoding_indices.shape[0], self.vocab_size, device=f_BChw.device) # [B*pn², V]
                encodings.scatter_(1, encoding_indices, 1)  # One-hot encoding [B*pn², V]

                # Store for later aggregation
                all_encodings.append(encodings)
                all_rest_NC.append(rest_NC)              
                ...
                # ​**Codebook Loss:**​ β * ||sg[f_hat] - f_BChw||² (Moves codebook vectors toward encoder outputs)
                # ​**Commitment Loss:**​ ||f_hat - sg[f_BChw]||² (Moves encoder outputs toward codebook)
                # Commitment Loss = ||encoder - codebook.detach()||² = ||f_hat - sg[f_BChw]||²
                # * Gradients only flow to encoder parameters
                # * Codebook remains fixed (updated separately via EMA)
                commitment_loss = F.mse_loss(f_BChw, f_hat.detach())
                mean_vq_loss += commitment_loss * self.beta
            
            mean_vq_loss *= 1. / SN
            f_hat = (f_hat.data - f_no_grad).add_(f_BChw)

            # Update cluster size statistics
            total_encodings = torch.cat(all_encodings, dim=0)  # [N_total, V]
            total_rest_NC = torch.cat(all_rest_NC, dim=0)      # [N_total, C]
            cluster_size = torch.sum(total_encodings.detach(), dim=0)  # [V]
            ...
            self.ema_cluster_size.data = (
                ...
                (n + self.vocab_size * self.epsilon) * n
            )

            # Update embedding weights
            dw = torch.matmul(total_encodings.t().detach(), total_rest_NC.detach())  # [V, C]
            self.ema_w.data.mul_(self.decay).add_(dw, alpha=1-self.decay)

            self.embedding.weight.data.copy_(
                self.ema_w.data / self.ema_cluster_size.data.unsqueeze(1)
            )
